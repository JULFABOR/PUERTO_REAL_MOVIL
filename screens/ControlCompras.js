...
const PurchaseForm = ({ isEdit, purchaseData, onSave, onClose, showAlert }) => {
  ...
  const handleAddItem = () => {
    if (itemName && itemPrice) {
      ...
      showAlert(t("purchases.error"), t("purchases.enterNameAndPrice"));
    }
  };
    ...
    <View style={styles.modalOverlay}>
      <View style={styles.modalContainer}>
        <View style={styles.modalHeader}>
          <Text style={styles.modalTitleText}>{isEdit ? t('purchases.editPurchase') : t('purchases.addPurchase')}</Text>
        ...
        <ScrollView style={styles.modalBody}>
          ...
          <TouchableOpacity style={styles.formButton} onPress={() => showAlert(t("purchases.info"), t("purchases.notImplemented"))}>
            <Text style={styles.formButtonText}>{proveedor || t('purchases.selectSupplier')}</Text>
          ...
          <View style={styles.formSection}>
            ...
            <View style={styles.addItemContainer}>
              <TextInput style={[styles.formInput, {flex: 2, marginRight: 8}]} placeholder={t('purchases.productName')} value={itemName} onChangeText={setItemName} />
              <TextInput style={[styles.formInput, {flex: 1}]} placeholder={t('purchases.price')} keyboardType="numeric" value={itemPrice} onChangeText={setItemPrice} />
        ...
        <TouchableOpacity style={styles.primaryButton} onPress={handleSave}>
          ...
          <Text style={styles.primaryButtonText}>{isEdit ? t('purchases.saveChanges') : t('purchases.addPurchase')}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
export default function ControlCompras({ navigation }) {
  ...
  const handleExportToPdf = () => { showAlert(t("purchases.info"), t("purchases.notImplemented")); };
    ...
    <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.overlay}>
          ...
          <View style={styles.controlsContainer}>
            <View style={styles.searchContainer}>
              ...
              <TextInput style={styles.searchInput} placeholder={t('purchases.search')} placeholderTextColor={DARK_GREY} value={searchQuery} onChangeText={setSearchQuery} />
            </View>
            <TouchableOpacity style={styles.addButton} onPress={() => setIsAddModalVisible(true)}><FontAwesome name="plus" size={20} color="#fff" /><Text style={styles.addButtonText}>{t('purchases.add')}</Text></TouchableOpacity>
          </View>
      ...
      <Modal visible={isViewModalVisible} onRequestClose={() => setIsViewModalVisible(false)} transparent={true} animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitleText}>{t('purchases.purchaseDetail')}</Text>
            ...
            {selectedPurchase && (
              <>
                <ScrollView style={styles.modalBody}>
                  <View style={styles.detailRow}><Text style={styles.detailLabel}>{t('purchases.code')}</Text><Text style={styles.detailValue}>{selectedPurchase.codigo}</Text></View>
                  <View style={styles.detailRow}><Text style={styles.detailLabel}>{t('purchases.supplier')}</Text><Text style={styles.detailValue}>{selectedPurchase.proveedor}</Text></View>
                  <View style={styles.detailRow}><Text style={styles.detailLabel}>{t('purchases.date')}</Text><Text style={styles.detailValue}>{selectedPurchase.fecha}</Text></View>
                  <View style={styles.detailRow}><Text style={styles.detailLabel}>{t('purchases.totalAmount')}</Text><Text style={[styles.detailValue, { color: TERRACOTTA, fontFamily: 'Roboto-Bold' }]}>${selectedPurchase.importe.toFixed(2)}</Text></View>
                  <Text style={[styles.detailLabel, { marginTop: 15, marginBottom: 5 }]}>{t('purchases.products')}:</Text>
                ...
                <TouchableOpacity style={styles.primaryButton} onPress={handleExportToPdf}><FontAwesome name="file-pdf-o" size={20} color="#fff" /><Text style={styles.primaryButtonText}>{t('purchases.exportToPdf')}</Text></TouchableOpacity>
              </>
            )}
          </View>
        </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            ...
            <View style={styles.modalBody}>
              ...
              {selectedPurchase && <Text style={styles.deleteInfo}>{t('purchases.recordWillBeDeleted', {code: selectedPurchase.codigo})}</Text>}
              <Text style={styles.deleteWarning}>{t('purchases.irreversibleAction')}</Text>
              <View style={styles.buttonContainer}>
                <TouchableOpacity style={[styles.button, styles.cancelButton]} onPress={() => setIsDeleteModalVisible(false)}><Text style={[styles.buttonText, {color: DARK_GREY}]}>{t('purchases.cancel')}</Text></TouchableOpacity>
                <TouchableOpacity style={[styles.button, styles.deleteButton]} onPress={confirmDeletePurchase}><Text style={styles.buttonText}>{t('purchases.delete')}</Text></TouchableOpacity>
              </View>
            </View>
          </View>
        </View>
